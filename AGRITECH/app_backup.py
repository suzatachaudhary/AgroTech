...
app.config['MAIL_USERNAME'] = 'arnabbhunia@gmail.com'  # Your Gmail address
...
def create_tables():
    with app.app_context():
        ...
        CREATE TABLE buy_sell_products (
            ...
            product_action ENUM('buy', 'sell') NOT NULL,
...
#routing for buy &sell


@app.route('/available_product')
def available_products():
    cursor = mysql.connection.cursor()
    cursor.execute("SELECT * FROM buy_sell_products WHERE product_action = 'buy'")
...
def sell_product():
    if request.method == 'POST':
        ...
        if not product_details:
            flash("Please provide product details.", 'danger')
            return redirect(url_for('sell_product'))

        # Insert the product as 'sell'
        cursor = mysql.connection.cursor()
        cursor.execute(''' 
            INSERT INTO buy_sell_products (user_name, location, product_action, product_details)
            VALUES (%s, %s, 'sell', %s)
        ''', (user_name, location, product_details))

        mysql.connection.commit()
        flash(f"Your product '{product_details}' has been listed for sale!", 'success')
