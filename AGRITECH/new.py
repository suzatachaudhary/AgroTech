...
app.config['MYSQL_PASSWORD'] = 'Arnab@2004'  # MySQL password
...
app.config['MAIL_USERNAME'] = 'arnabbhunia1609@gmail.com'  # Your Gmail address
app.config['MAIL_PASSWORD'] = 'arnab2004'  # Or App Pass
app.config['MAIL_DEFAULT_SENDER'] = 'arnabbhuia1609@gmail.com'
...
# #routing for book tools services
...
def tools_home():
    ...
    # Get **current available tools** for rent
    cursor.execute("SELECT * FROM tools WHERE available = 1")
    available_tools = cursor.fetchall()

    # Get **ongoing bookings** for the logged-in farmer
    cursor.execute("SELECT * FROM bookings WHERE farmer_id = %s AND status = 'Ongoing'", (user_id,))
    farmer_bookings = cursor.fetchall()

    # Get **ongoing bookings** for the logged-in owner
    cursor.execute("SELECT * FROM bookings WHERE owner_id = %s AND status = 'Ongoing'", (user_id,))
    owner_bookings = cursor.fetchall()

    # Get **notifications** for new bookings
    cursor.execute("""
        SELECT * FROM bookings 
        WHERE (farmer_id = %s OR owner_id = %s) 
        AND status = 'Pending'
    """, (user_id, user_id))
    ...
    return render_template(
        ...
        farmer_bookings=farmer_bookings,
        owner_bookings=owner_bookings,
        notifications=notifications
    )

@app.route('/add_tool_page')
def add_tool_page():
    return render_template("tools_add.html")
...
def add_tool():
    ...
    cursor = mysql.connection.cursor()
    cursor.execute(
        "INSERT INTO tools (owner_id, name, hourly_rate, location, available) VALUES (%s, %s, %s, %s, %s)",
        (data['owner_id'], data['name'], data['hourly_rate'], data['location'], True)
    )
    mysql.connection.commit()
    cursor.close()
    return redirect(url_for('tools_home'))

@app.route('/book_tool', methods=['POST'])
def book_tool():
    data = request.form
    ...
    cursor = mysql.connection.cursor()
    cursor.execute(
        "INSERT INTO bookings (farmer_id, tool_id, start_time, end_time, total_price, verification_code) VALUES (%s, %s, %s, %s, %s, %s)",
        (data['farmer_id'], data['tool_id'], data['start_time'], data['end_time'], data['total_price'], verification_code)
    )
    mysql.connection.commit()
    cursor.close()
    
    return jsonify({"message": "Booking request sent!", "verification_code": verification_code})
@app.route('/payment', methods=['POST'])
def process_payment():
    data = request.json
    payment = razorpay_client.order.create(dict(amount=int(data['amount'] * 100), currency="INR", payment_capture="1"))
    
    cursor = mysql.connection.cursor()
    cursor.execute("INSERT INTO payments (booking_id, amount, status) VALUES (%s, %s, %s)",
                   (data['booking_id'], data['amount'], "Pending"))
    mysql.connection.commit()
    cursor.close()
...
def tools_home_live():
    ...
    # Fetch ongoing bookings (for farmers)
    cursor.execute("SELECT * FROM bookings WHERE farmer_id = %s AND status = 'Ongoing'", (user_id,))
    farmer_bookings = cursor.fetchall()

    # Fetch ongoing bookings (for owners)
    cursor.execute("SELECT * FROM bookings WHERE owner_id = %s AND status = 'Ongoing'", (user_id,))
    owner_bookings = cursor.fetchall()
...
def request_booking():
    ...
    cursor.execute(
        "INSERT INTO bookings (farmer_id, tool_id, start_time, end_time, total_price, status) VALUES (%s, %s, %s, %s, %s, 'Pending')",
        (data['farmer_id'], data['tool_id'], data['start_time'], data['end_time'], data['total_price'])
    )
    
    mysql.connection.commit()
    cursor.close()
    
    return jsonify({"message": "Booking request sent, waiting for owner approval."})
...
def approve_booking():
    ...
    if action == "accept":
        ...
        message = "Booking approved. Farmer can now proceed with payment."
    else:
...
def create_payment():
    ...
    cursor = mysql.connection.cursor()
    cursor.execute("SELECT total_price FROM bookings WHERE id = %s", (booking_id,))
    result = cursor.fetchone()

    if not result:
        return jsonify({"error": "Booking not found"}), 404

    total_price = int(result[0] * 100)  # Convert to paise (Razorpay requires amounts in paise)
...
def confirm_payment():
    ...
    cursor = mysql.connection.cursor()
    cursor.execute("UPDATE bookings SET payment_status = 'Paid', payment_id = %s WHERE id = %s", 
                   (payment_id, booking_id))
    mysql.connection.commit()
    cursor.close()

    return jsonify({"message": "Payment successful! Booking confirmed."})
def generate_code():
...
def generate_verification_code():
    ...
    return_code = generate_code()

    cursor = mysql.connection.cursor()
    cursor.execute("UPDATE bookings SET delivery_code = %s, return_code = %s WHERE id = %s", 
                   (delivery_code, return_code, booking_id))
    mysql.connection.commit()
    cursor.close()

    return jsonify({"message": "Verification codes generated!"})

@app.route('/verify_delivery', methods=['POST'])
def verify_delivery():
    data = request.json
    booking_id = data['booking_id']
    entered_code = data['delivery_code']

    cursor = mysql.connection.cursor()
    cursor.execute("SELECT delivery_code FROM bookings WHERE id = %s", (booking_id,))
    result = cursor.fetchone()

    if not result or result[0] != entered_code:
        return jsonify({"error": "Invalid delivery code!"}), 400

    cursor.execute("UPDATE bookings SET booking_status = 'Ongoing' WHERE id = %s", (booking_id,))
    mysql.connection.commit()
    cursor.close()

    return jsonify({"message": "Tool delivery confirmed! Rental started."})

@app.route('/verify_return', methods=['POST'])
def verify_return():
    data = request.json
    booking_id = data['booking_id']
    entered_code = data['return_code']

    cursor = mysql.connection.cursor()
    cursor.execute("SELECT return_code, payment_status FROM bookings WHERE id = %s", (booking_id,))
    result = cursor.fetchone()

    if not result or result[0] != entered_code:
        return jsonify({"error": "Invalid return code!"}), 400

    cursor.execute("UPDATE bookings SET booking_status = 'Completed' WHERE id = %s", (booking_id,))
    mysql.connection.commit()
    cursor.close()

    return jsonify({"message": "Tool return confirmed! Owner will receive payment."})
@app.route('/owner_dashboard/<int:owner_id>')
def owner_dashboard(owner_id):
    cursor = mysql.connection.cursor()
    cursor.execute("""
        SELECT b.id, u.name AS farmer_name, t.name AS tool_name, b.start_time, b.end_time, b.total_price, 
               b.booking_status, b.owner_approval, b.payment_status
        FROM bookings b
...
def approve_booking():
    data = request.json
    booking_id = data['booking_id']
    approval_status = data['approval_status']  # 'Approved' or 'Rejected'

    cursor = mysql.connection.cursor()
    cursor.execute("UPDATE bookings SET owner_approval = %s WHERE id = %s", (approval_status, booking_id))
    mysql.connection.commit()
    cursor.close()

    return jsonify({"message": f"Booking {approval_status.lower()}!"})
...
def process_payment():
    ...
    # Get total price and owner ID
    cursor = mysql.connection.cursor()
    cursor.execute("""
        SELECT b.total_price, t.owner_id, u.email 
        FROM bookings b 
        JOIN tools t ON b.tool_id = t.id 
    ...
    total_price, owner_id, owner_email = booking
    ...
    mysql.connection.commit()
    cursor.close()
@app.route('/generate_payment_order', methods=['POST'])
def generate_payment_order():
    ...
    cursor = mysql.connection.cursor()
    cursor.execute("""
        SELECT total_price, owner_approval 
        FROM bookings 
        WHERE id = %s
    """, (booking_id,))
    booking = cursor.fetchone()
    ...
    total_price, owner_approval = booking

    # Ensure booking is approved before payment
    if owner_approval != "Approved":
        return jsonify({"error": "Owner has not approved this booking yet"}), 403
    ...
    return jsonify({"order_id": order["id"], "amount": total_price})
...
def confirm_payment():
    ...
    return jsonify({"message": "Payment successful!", "payment_id": payment_id})
