    ...
    if request.method == 'POST':
        ...
        try:
            msg = Message(
                ...
                sender=email,
                recipients=['arnabbhunia1609@gmail.com']
            )
...
#     if request.method == 'POST':
#         farming_name = request.form['farming_name']
#         labor_needs = request.form['labor_needs']
#         tools_requirements = request.form['tools_requirements']
#         pesticide_requirements = request.form['pesticide_requirements']
#         risk_factors = request.form['risk_factors']
#         market_value = request.form['market_value']
#         growth_trend = request.form['growth_trend']
#         image_url = request.form['image_url']

#         # Insert data into the farming_data table
#         cur = mysql.connection.cursor()
#         cur.execute('''
#             INSERT INTO farming_data (farming_name, labor_needs, tools_requirements, pesticide_requirements, 
#                                       risk_factors, market_value, growth_trend, image_url)
#             VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
#         ''', (farming_name, labor_needs, tools_requirements, pesticide_requirements, risk_factors,
#               market_value, growth_trend, image_url))
#         mysql.connection.commit()
#         cur.close()

#         flash('Farming data added successfully!', 'success')
#         return redirect(url_for('add_farming_data_form'))
    
#     return render_template('add_farming_data.html')
...
def tools_home():
    ...
    cursor.execute("SELECT * FROM bookings WHERE farmer_id=%s", (user_id,))
    my_rentals = cursor.fetchall()
    
    cursor.execute("SELECT * FROM bookings WHERE owner_id=%s", (user_id,))
    my_given_rentals = cursor.fetchall()
    ...
    return render_template('tools_home.html', 
                           ...
                           my_given_rentals=my_given_rentals, 
                           user_data=user_data)
...
def add_tool():
    try:
        ...
        # Validate input
        if not data or not all(k in data for k in ["owner_id", "name", "hourly_rate", "location"]):
            return jsonify({"message": " Missing required fields!"}), 400

        if not isinstance(data["hourly_rate"], (int, float)) or data["hourly_rate"] <= 0:
            return jsonify({"message": " Hourly rate must be a positive number!"}), 400

        # Database Connection
        db = get_db_connection()
        cursor = db.cursor()

        # Insert tool details into database
        query = "INSERT INTO tools (owner_id, name, hourly_rate, location, available) VALUES (%s, %s, %s, %s, 1)"
        values = (data["owner_id"], data["name"], data["hourly_rate"], data["location"])
        ...
        return jsonify({"message": "✅ Tool added successfully!", "success": True}), 201

    except Exception as e:
        print("Error:", str(e))  # Log the error for debugging
        return jsonify({"message": " Something went wrong!", "error": str(e)}), 500

@app.route('/available_tools', methods=['GET'])
def available_tools():
    cur = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cur.execute("SELECT id, name, hourly_rate, location FROM tools WHERE available = 1")
    tools = cur.fetchall()
    cur.close()
    return render_template("tools_available.html", tools=tools)

@app.route("/your_listed_tools", methods=["GET"])
def your_listed_tools():
    """Fetches tools listed by the logged-in owner and renders HTML"""
    if "user_id" not in session:
        return "<h3>❌ You must be logged in!</h3>", 401
    ...
    try:
        ...
        query = "SELECT id, name, hourly_rate, location, available FROM tools WHERE owner_id = %s"
        ...
        return render_template("your_listed_tools.html", tools=tools)

    except Exception as e:
        print("Error:", str(e))
        return "<h3>❌ Failed to fetch tools!</h3>", 500
    
@app.route("/update_tool", methods=["POST"])
def update_tool():
    """Update the hourly rate of a listed tool"""
    if "user_id" not in session:
        return jsonify({"error": "Unauthorized"}), 401

    owner_id = session["user_id"]
    tool_id = request.form.get("tool_id")
    new_hourly_rate = request.form.get("new_hourly_rate")

    if not tool_id or not new_hourly_rate:
    ...
    try:
        ...
        query = "UPDATE tools SET hourly_rate = %s WHERE id = %s AND owner_id = %s"
        cursor.execute(query, (new_hourly_rate, tool_id, owner_id))
        db.commit()

        cursor.close()
        db.close()

        return jsonify({"success": True, "message": "Hourly rate updated successfully!"})

    except Exception as e:
        print("Error:", str(e))
        return jsonify({"error": "Failed to update hourly rate"}), 500

@app.route("/delete_tool", methods=["POST"])
def delete_tool():
    """Delete a tool listed by the owner"""
    if "user_id" not in session:
        return jsonify({"error": "Unauthorized"}), 401
    ...
    try:
        ...
        query = "DELETE FROM tools WHERE id = %s AND owner_id = %s"
        cursor.execute(query, (tool_id, owner_id))
        db.commit()
